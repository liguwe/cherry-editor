...
const linkDomMutation = (editor: Editor, attachState: AttachState, data: LinkDialogOutput, elm: Element) => {
  const selectedElm = elm || editor.selection.getNode();
...
const link = (editor: Editor, attachState: AttachState, data: LinkDialogOutput, elm: Element = null) => {
  editor.hasPlugin('rtc', true) ? editor.execCommand('createlink', false, unwrapOptions(data)) : linkDomMutation(editor, attachState, data, elm);
};
...
const inTable = function (editor: Editor) {
  if (!editor.selection) {
    return false;
  }
  const node = <any>editor.selection.getStart();
...
const getEditableLink = (editor:Editor, condition: string, conditionData: string) => {
  const root = editor.selection.getNode();
  if (root.getAttribute(condition) === conditionData) {
    return root;
  } else if ((root.parentNode as Element).getAttribute(condition) === conditionData) {
    return root.parentNode;
  } else {
    return root.querySelectorAll(`a[${condition}="${conditionData}"]`);
  }
};

const isLinkUneditable = (editor: Editor, disableMode: string[]) : boolean => {
  const root = editor.selection.getNode();
  return findUneditableLink(root) || false; // 表示选中的区域中包不包含不可编辑的链接
  ...
  function findUneditableLink(node) {
    ...
    if (node.nodeName === 'A' && node.getAttribute('is-tapdlink') && disableMode.indexOf(node.getAttribute('mode')) > -1) {
      ...
      const uneditableList = node.querySelectorAll('a[is-tapdlink="true"]');
      if (!uneditableList.length) {
        return false;
      }
      uneditableList.forEach((item) => {
