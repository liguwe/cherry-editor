...
const cherryPlugins = fs.readdirSync(pluginsDir).filter(p => {
  return p.indexOf('cherry') > -1 && p.indexOf('cherry-plugins') < 0;
});
...
const warninngHandler = (warning) => {
  if ( warning.code === 'THIS_IS_UNDEFINED' ) { return; }
  if ( warning.code === 'MISSING_EXPORT' ) {
    if (
      warning.message.indexOf('sanitize') > -1
...
module.exports = function (grunt) {
  ...
  var cherryPackageData = grunt.file.readJSON('cherry-package.json');
    ...
    rollup: Object.assign(
      ...
      },
      {
        cherry: {
          options: {
            ...
            plugins: [
              swag.nodeResolve({
                ...
                prefixes: {
                  'tinymce/core': 'lib/core/main/ts',
                  'tinymce/icons/default': '../oxide-icons-default/dist/icons/default',
                  ...themes.reduce((paths, name) => {
                    paths[`tinymce/themes/${name}`] = `lib/themes/${name}/main/ts`;
                    return paths;
                  }, {}),
                  ...plugins.reduce((paths, name) => {
                    paths[`tinymce/plugins/${name}`] = `lib/plugins/${name}/main/ts`;
                    return paths;
                  }, {}),
                }
              }),
          ...
          files:[
            {
              src: 'lib/core/main/ts/api/cherry/Main.js',
              dest: 'js/cherry/cherry-editor.js',
              sourcemap: false,
            }
          ]
        },
        'cherry-plugins': {
          ...
          files:[
            { src: `lib/plugins/cherry-plugins/main/ts/Main.js`,
              dest: `js/cherry/plugins/cherry-plugins/plugin.js`
            }
          ]
        },
      },
      ...
      gruntUtils.generate(plugins, 'plugin--cherry', (name) => {
        return {
          ...
          files:[ { src: `lib/plugins/${name}/main/ts/Main.js`, dest: `js/cherry/plugins/${name}/plugin.js` } ]
        };
      }),
    ...
    uglify: Object.assign(
      {
        ...
        core: {
          files: [
            ...
            { src: 'js/cherry/cherry-editor.js', dest: 'js/cherry/cherry-editor.min.js' },
      ...
      gruntUtils.generate(plugins, 'plugin--cherry', (name) => {
        var pluginExtras = {
          emoticons: [
            { src: 'src/plugins/emoticons/main/js/emojis.js', dest: 'js/cherry/plugins/emoticons/js/emojis.min.js' },
            { src: 'src/plugins/emoticons/main/js/emojiimages.js', dest: 'js/cherry/plugins/emoticons/js/emojiimages.min.js' }
          ]
        };
        return {
          files: [
            { src: `js/cherry/plugins/${name}/plugin.js`, dest: `js/cherry/plugins/${name}/plugin.min.js` }
          ].concat(pluginExtras.hasOwnProperty(name) ? pluginExtras[name] : [])
        };
      }),
      gruntUtils.generate(themes, 'theme', (name) => {
        ...
        files: [
          {
            ...
            cwd: '../oxide/build/skins/content',
            src: '**',
            dest: 'js/cherry/skins/content'
          },
          {
            expand: true,
            cwd: '../oxide-icons-default/dist/icons/default',
            src: '**',
            dest: 'js/cherry/icons/default'
          },
          ...gruntUtils.flatMap(oxideUiSkinMap, function (name, mappedName) {
            return [
              {
                expand: true,
                cwd: '../oxide/build/skins/ui/' + name,
                src: '**',
                dest: 'js/cherry/skins/ui/' + mappedName
              }
            ];
          })
        ]
      ...
      component_cherry: {
        options: {
          ...
          onBeforeSave: function (zip) {
            ...
            zipUtils.addIndexFiles(
              zip,
              getDirs('js/cherry/icons'),
              zipUtils.generateIndex('icons', 'icons')
            );
          },
          to: 'dist/cherry-editor-package.zip',
          dataFilter: (args) => {
            if (args.filePath.endsWith('.min.css')) {
              args.data = stripSourceMaps(args.data);
            }
        ...
        src: [
          ...
          'js/cherry/icons',
          'js/cherry/plugins',
          'js/cherry/cherry-editor.js',
          'js/cherry/cherry-editor.min.js',
          'js/cherry/license.txt',
        ]
      },
    },
  ...
  grunt.registerTask('symlink-distfiles', 'Links built dist content to the root directory', function () {
    // Windows doesn't support symlinks, so copy instead of linking
    if (grunt.file.exists('../../dist')) grunt.file.delete('../../dist', { force: true });
    if (grunt.file.exists('../../js')) grunt.file.delete('../../js', { force: true });
    if (grunt.file.exists('./dist'))  grunt.file.copy('dist', '../../dist');
    if (grunt.file.exists('./js'))  grunt.file.copy('js', '../../js');
    grunt.log.write('Copied 2 directories');
  });
  ...
  grunt.registerTask('prod:cherry', [
    'clean:dist',
    'shell:tsc',
    'globals',
    'emoji',
  ...
  grunt.registerTask('start:cherry', [
    'dev',
    'webpack-dev-server'
  ]);
